...
const SESSION_KEY = 'fof_intro_played';
...
export const initIntro = () => {
  const overlay = document.getElementById('fof-intro');
  if (!overlay) return;

  // Repeat sessions / reduced motion are handled by the inline head script.
  if (
    document.documentElement.classList.contains('fof-intro-skip') ||
  ...
  const stage = overlay.querySelector('.fof-intro__stage');
  const bagGroup = overlay.querySelector('.fof-intro__bag-group');
  const shadow = overlay.querySelector('.fof-intro__shadow');
  const ink = Array.from(overlay.querySelectorAll('.fof-intro__ink'));
  const ring = overlay.querySelector('.fof-intro__ring');
  const arc = overlay.querySelector('.fof-intro__arc');
  const shine = overlay.querySelector('.fof-intro__shine');
  const word = overlay.querySelector('.fof-intro__word');
  ...
  document.body.classList.add('fof-intro-active');
  ...
  const finish = (instant = false) => {
    ...
    markPlayed();
    document.body.classList.remove('fof-intro-active');

    const cleanup = () => {
      overlay.remove();
      document.dispatchEvent(new CustomEvent('fof:intro-complete'));
    };
  ...
  tl.fromTo('.fof-intro__bg', { opacity: 0 }, { opacity: 1, duration: 0.6 }, 0);
  ...
  // 7. The F>F wordmark rises in beneath the mark.
  if (word) {
