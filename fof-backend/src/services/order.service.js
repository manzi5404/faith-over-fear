...
async function createFromCart(userId, customerData, sessionId = null) {
  ...
  for (const item of cartItems) {
    ...
    const unitPrice = v.price_override || (p.price_categories ? parseFloat(p.price_categories.price) : null) || p.price;
    enrichedItems.push({
      variantId: v.id,
      productId: p.id,
      productName: p.name,
...
async function createDirect(userId, customerData, items, sessionId = null) {
  ...
  for (const item of items) {
    ...
    const unitPrice = variant.price_override || (product.price_categories ? parseFloat(product.price_categories.price) : null) || product.price;
