...
const res = await fetch(`${API_BASE_URL}/api/contact`, {
    ...
    getCachedProduct(productId) {
        try {
            const cached = localStorage.getItem('DOTTIE_product_cache');
            if (cached) {
                const products = JSON.parse(cached);
                const now = Date.now();
                const filtered = products.filter(p => now - (p._cachedAt || 0) < 3600000);
                if (filtered.length > 0) {
                    localStorage.setItem('DOTTIE_product_cache', JSON.stringify(filtered));
    ...
    cacheProducts(products) {
        try {
            ...
            localStorage.setItem('DOTTIE_product_cache', JSON.stringify(cached));
        } catch (e) {
